...
def test_user_role_relationship(setup_database):
    ...
    user1 = DimUser.create(name_user='Ana', email='ana@email.com', password='123', id_role=role)
    user2 = DimUser.create(name_user='Bruno', email='bruno@email.com', password='456', id_role=role)

    users = list(role.users)  # backref de ForeignKey
    assert len(users) == 2
    assert users[0].name_user == 'Ana'
    assert users[1].name_user == 'Bruno'
